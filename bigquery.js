...
async function buildAdlensData() {
  ...
  const [medioRows, grupoRows, agenciaRows, sectorRows, mesRows, anunMediosRows, factRows, mmiRows, anunRangoRows, evolucionRows, mmiClusterRows, clusterGrupoRows, rubroRows, clusterAgenciaRows, radaDimRows, radaMetricRows, evolucionDetalleRows, estacionalidadDetalleRows, mediosDetalleRows] = await Promise.all([
    ...
    // Medios detalle: inversión por anunciante × grupo × medio × vehículo × agencia (para filtros de tabs Medios y Agencias)
    query(`SELECT m.anunciante AS nombre, m.GrupoEmpresarial AS gm, m.Medio AS medio, m.Vehiculo AS vehiculo, m.Agencia AS agencia, ROUND(SUM(m.RANGODEINVERSION),0) AS v FROM ${M} m ${innerJoin} WHERE m.Medio IS NOT NULL GROUP BY m.anunciante, m.GrupoEmpresarial, m.Medio, m.Vehiculo, m.Agencia`).catch(()=>[]),
  ]);
  ...
  return {
    ...
    medios_detalle: mediosDetalleRows.map(r => ({ nombre: fixName(r.nombre), gm: r.gm||'', medio: fixMedio(r.medio||''), vehiculo: (r.vehiculo||'').toString().trim(), agencia: fixAgencia(r.agencia||''), v: +r.v })),
