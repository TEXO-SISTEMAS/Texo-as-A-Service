...
app.post('/api/chat/save', async (req, res) => {
  try {
    const { messages, title, agencia, userEmail, nombre: bodyNombre, id: existingId } = req.body;
    if (!messages?.length) return res.status(400).json({ error: 'messages requerido' });
    const payload = { title, messages, agencia: agencia || null, userEmail: userEmail || null, saved_at: new Date().toISOString() };
    let saved;
    if (existingId) {
